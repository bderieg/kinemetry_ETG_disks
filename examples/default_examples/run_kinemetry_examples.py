...
def vel_map_kinemetry():
    """
    This procedure ilustrates a simple way of calling kinemetry to analyse a 
    velocity map, and plot some relevant outputs    
    """


    file_dir = path.dirname(path.realpath(kin.__file__))
    file = file_dir + '/examples/NGC2974_SAURON_kinematics.dat'
    num, xbin, ybin, velbin, er_velbin, sigbin, er_sigbin = np.genfromtxt(file, unpack=True)

    # call kinemetry
    start=time.time()
    k = kinemetry(xbin, ybin, velbin, error=er_velbin, scale=0.8, name='NGC2974')
    end=time.time()
    print('finshed in ', end - start, ' seconds')
    
    #make radial plots
    plot_kinemetry_profiles_velocity(k, name='NGC2974 SAURON V-map')
...
def vel_map_centre_kinemetry():
    ...
    # call kinemetry
    start=time.time()
    k = kinemetry(xbin, ybin, velbin, fixcen=False, x0=0.5, y0=0.5, scale=0.8, name='NGC2974')
    end=time.time()
    print('finshed in ', end - start, ' seconds')
    
    #make radial plots
    plot_kinemetry_profiles_velocity(k, fitcentre=True, name='NGC2974 SAURON V-map, fit centre')
...
def vel_map_ellipse_kinemetry():
    ...
    rad = rad/0.8 # move to pixel scale

    pa = np.array([-89.25857204,  72.        ,  45.85858326,  41.29380557,
    ...
    # call kinemetry
    start=time.time()
    k = kinemetry(xbin, ybin, velbin, paq=PAQ, radius=rad, scale=1, name='NGC2974', plot=False)
    end=time.time()
    print('finshed in ', end - start, ' seconds')
    
    #make radial plots
    plot_kinemetry_profiles_velocity(k, fitcentre=True, name='NGC2974 V-map, fixed ellipses')
...
def sig_map_kinemetry():
    ...
    # call kinemetry
    start=time.time()
    k = kinemetry(xbin, ybin, sigbin, error=er_sigbin, even=True, ntrm=8, 
                  scale=0.8, rangePA=[40,50], rangeQ=[0.5,0.99], name='NGC2974')
    end=time.time()
    print('finshed in ', end - start, ' seconds')
    
    #make radial plots
    plot_kinemetry_profiles_sigma(k, name='NGC2974 SAURON, $\sigma$-map')
...
def sig_map_centre_kinemetry():
    ...
    # call kinemetry
    start=time.time()
    k = kinemetry(xbin, ybin, sigbin, error=er_sigbin, even=True, ntrm=8, scale=0.8, 
              rangePA=[40,50], rangeQ=[0.5,0.99], x0=0.5, y0=0.5, fixcen=False,
              name='NGC2974')
    end=time.time()
    print('finshed in ', end - start, ' seconds')
    
    #make radial plots
    plot_kinemetry_profiles_sigma(k, fitcentre=True, name='NGC2974 SAURON, $\sigma$-map, fit centre')
