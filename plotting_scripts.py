...
def plot_kinemetry_profiles(k):
    ...
    q = k.q[:term]
    er_q = k.er_q[:term]
    k1 = np.sqrt(k.cf[:term,1]**2 + k.cf[:term,2]**2)
    er_k1 = 1 - (np.sqrt(k.cf[:term,1]**2 + k.cf[:term,2]**2))/np.sqrt((k.cf[:term,1]+k.er_cf[:term,1])**2 + (k.cf[:term,2]+k.er_cf[:term,2])**2)
    k5 = np.sqrt(k.cf[:term,5]**2 + k.cf[:term,6]**2)
    er_k5 = 1 - (np.sqrt(k.cf[:term,5]**2 + k.cf[:term,6]**2))/np.sqrt((k.cf[:term,5]+k.er_cf[:term,5])**2 + (k.cf[:term,6]+k.er_cf[:term,6])**2)
    k5k1 = k5/k1
    er_k5k1 = 1 - (np.sqrt(k.cf[:term,1]**2+k.cf[:term,2]**2)*np.sqrt((k.cf[:term,5]+k.er_cf[:term,5])**2+(k.cf[:term,6]+k.er_cf[:term,6])**2))/(np.sqrt((k.cf[:term,1]+k.er_cf[:term,1])**2+(k.cf[:term,2]+k.er_cf[:term,2])**2)*np.sqrt(k.cf[:term,5]**2+k.cf[:term,6]**2))
